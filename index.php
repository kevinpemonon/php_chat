...
<body>
    <div id="main">
        ...
        //$lastname = $db->query('SELECT pseudo FROM message ORDER BY id DESC LIMIT 1')->fetch();
        ...
        $reponse = $db->query('SELECT pseudo, contenu, DATE_FORMAT(date_message, "%d/%m/%Y à %Hh%imin%ss") AS date FROM messages ORDER BY id DESC LIMIT 5')->fetchAll();
                        ...
                        foreach ($reponse as $value)
                        {
                            ...
                            if ($compteur == count($reponse))
                                ...
                                echo '      <p class="reponse_pseudo">'. htmlspecialchars($value['pseudo']) . ' : </p>
                                            <p class="reponse_message">' . htmlspecialchars($value['contenu']) . '</p>
